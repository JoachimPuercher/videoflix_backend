...
class RegistrationView(generics.CreateAPIView):
    """POST /api/register/: create inactive user and mail activation link."""
...
class LogoutView(TokenBlacklistView):
    ...
    def post(self, request, *args, **kwargs) -> Response:
        ...
        try:
            ...
            response = Response(
                {"detail": LOGOUT_MESSAGE}, status=status.HTTP_200_OK
            )
...
class CookieTokenRefreshView(TokenRefreshView):
    """POST /api/token/refresh/: refresh cookie in, new access cookie out."""
...
class UserActivationView(views.APIView):
    """GET /api/activate/<uidb64>/<token>/: activate account from mail link.
        ...
        try:
            ...
            if default_token_generator.check_token(user, token) is True:
                ...
                if request.accepted_renderer.format == 'html':
                    ...
                    return Response(
                        template_data, template_name='activation_result.html'
                    )
                else:
                    return Response(
                        {"message": "Account successfully activated."},
...
class PasswordConfirmView(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Field errors (missing or mismatching passwords) are rendered by DRF
        # as 400.
        serializer.is_valid(raise_exception=True)
        serializer.save()
